...
class AlexNet(nn.Module):
    def __init__(self, num_classes=1000, drop_out=0.5):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            # 输入 196x27x27 输出 196x13x13
            nn.MaxPool2d(kernel_size=3, stride=2),
            # 输入 196x13x13 输出 384x13x13
...
class LW_AlexNet(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.features = nn.Sequential(
            ...
            # 输入 64x55x55 输出64x27x27
            nn.MaxPool2d(kernel_size=3, stride=2),
            CBAMModule(64),  # Add CBAM module after the second convolutional layer
            nn.GroupNorm(8, 64),
            # 输入 64x27x27 输出 128x27x27
            # nn.Conv2d(64, 128, kernel_size=5, padding=2),
            # 输入 128x27x27 输出 256x27x27
            InceptionModule5x5(64),
            nn.ReLU(inplace=True),
